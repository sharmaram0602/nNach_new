(function (global, factory) {
  ...
  const towFAVerificarionInit = () => {
    const verificationForm = document.querySelector('[data-2FA-varification-login]');
    const inputFields = document.querySelectorAll(
      '[data-2FA-varification-login] input[type=number]'
    );
  ...
  const towFAVerificarionInitForget = () => {
    const verificationForm = document.querySelector('[data-2FA-varification-forget]');
    const inputFields = document.querySelectorAll(
      '[data-2FA-varification-forget] input[type=number]'
    );
  ...
  docReady(towFAVerificarionInitForget);

  docReady(() => {
 
    
    const selectedTransactionRowsBtn = document.querySelector('[data-selected-rows-transaction-schedule]');
    const selectedRows = document.getElementById('selectedRows');
 
    if (selectedTransactionRowsBtn) {
      const bulkSelectEl = document.getElementById('bulk-select-transaction-schedule');
      const bulkSelectInstance =
        window.phoenix.BulkSelect.getInstance(bulkSelectEl);
      selectedTransactionRowsBtn.addEventListener('click', () => {
        selectedRows.innerHTML = JSON.stringify(
          ...
          2
            ...
            const selectedRowsLoop = bulkSelectInstance.getSelectedRows();
              selectedRowsLoop.forEach((row) => {
                if(row.mts_id){
            ...
            for (var i = 0; i < selectedTransactions.length; i++) {
              ...
              var x=i+1;
               ...
               bulkhtml+='<td class="order py-2  ps-3 align-middle white-space-nowrap srno"><a class="fw-semi-bold" href="#">#'+x+' </a></td>'+
                      
                  '<td class="align-middle ps-3">'+selectedTransactions[i].date+'</td>'+
                  '<td class="align-middle ps-3">'+selectedTransactions[i].amount+'</td>';
          ...
          $('#userDataBulkSkip').html(bulkhtml); 

          $('#skipModalBulk').modal('show');
