                 ...
                 <?php if(in_array('createMandate', $user_permission)):?> 
                    <div class="ms-xxl-auto ms-xl-auto ms-lg-auto ms-md-auto ms-sm-auto">
                      <!--  <button class="btn btn-link text-900 me-4 px-0" id="showOptionsButton"><span class="fas fa-eye-slash fs--1 me-2"></span>Show Options</button>
                       ...
                       <a class="btn btn-warning btn-sm" id="pendingMandates" href="<?php echo base_url('Users/mandateCustomer/');?>"><span class="fas fa-eye me-2"></span>View All</a>
        ...
        $("#customer_end_date").change(function() {
            ...
            for (var k = 0; k < schedule.length; k++) {
                ...
                schedule_html += '<tr>' +
                    ...
                    '<td><input type="number" class="form-control form-control-sm" id="amount" name="amount" value="" ></td>' +
                    '</tr>';
            }
    ...
    return `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
                ...
                function showDynamicSchedule(mandate_customer_id)
                {
                            ...
                            success: function(response) {
                              if (response.status) {
                    var data = response.data;  
                    // console.log(data.transactionScheduleData); 

                    var schedule_html = '';
                    var schedule = JSON.parse(data.transactionScheduleData); 
...
// var formattedDateDisplay = formatDateToDMY(scheduleData[k].emi_date);
                    ...
                    $('#ScheduleTable tbody').on('blur', 'input[name="loan_amount"]', function() { 
        ...
        dataTable.search(searchTerm).draw();
         ...
         function showUsersData(){
            ...
            dataTable =   $('#dataTable').DataTable({
                    ...
                    "ajax": {
                         'type': 'GET',
                         'url': "<?php echo base_url() ?>api/User/showPendingMandateUsersList/",
                         'async': false,
                         'method':'get',
                         'dataType': 'json',
                         'dataSrc': 'data',
    ...
    function getMandateVerificationExpiryTime(mandate_customer_id) {
       ...
       $('#userDataMandate').on('click', '.item-edit', function(){
         var mandate_customer_id = $(this).attr('data-mandate-customer-id');
          ...
          // calculateEndDate();
          showDynamicSchedule(mandate_customer_id);
       ...
       function mandateCustomerRowFetch(mandate_customer_id){
         ...
         $.ajax({
               ...
               success: function(response){
                ...
                if (data.mandate_status_message === 'User Aborted' || data.mandate_status_message === 'failure' || data.mandate_status_message==="" || data.mandate_status_message===null || data.mandate_status_message==="null") {
        ...
        async function mandateFormSave() {
                      ...
                      // loan
                        ...
                        formData.append('amount', $('#loan_amount').val());
