<div class="content" id="showChangeMandateData">
    <div class="col-12 col-xl-12 col-xxl-12">
            <div class="card h-100">
                <div class="card-body">
                  ...
                  <p class="text-700 mb-4">Recent activity across mandate</p>
                  <div class="timeline-vertical timeline-with-details">
                        <div class="col-sm-12 col-md-12  col-lg-12 col-xl-12 col-xxl-12">
                                <div class="card h-100">
                                    <div class="card-body">
                                        <div class="timeline-vertical timeline-with-details"  id="mandateLogContainer">
...
<script type="text/javascript">
   ...
   $(document).ready(function() {
    ...
    function fetchAndDisplayMandateLogs(page) {
        $.ajax({
            type: 'GET',
            url: "<?php echo base_url() ?>api/Mandate_Log/" + page,
            data: {
            ...
            success: function(response) {
                ...
                for (var i = 0; i < data.length; i++) {
                  if(data[i].mandate_log_event=='INSERT'){
                      action_shown= "Added";

                  }else if(data[i].mandate_log_event=='UPDATE'){
                    action_shown= "Updated";
                    
                  }else if(data[i].mandate_log_event=='DELETE'){
                    action_shown= "Deleted";

                  }else if(data[i].mandate_log_event=='OTHER'){
                    action_shown= "Other Action";

                  }
                 
                    var customer_name = data[i].customer_name;
                    var mandate_previous_data = data[i].mandate_previous_data;
                    var mandate_current_data = data[i].mandate_current_data;
                    var firstname = data[i].firstname;
                    var middlename = data[i].middlename;
                    var lastname = data[i].lastname;
                    var table='';
                  
                    if(data[i].mandate_log_event=='UPDATE' || data[i].mandate_log_event=='OTHER') {
                           ...
                           table+='<div class="table-responsive table-reponsive-sm scrollbar"><table class="table table-sm table-bordered">'+
                                  ...
                                  for (var key in mandate_current_data) {   
                                      var prev_val='';
                                      var current_val='';
                                      if(mandate_previous_data){
                                            prev_val=mandate_previous_data[key];  

                                      } 
                                      if(mandate_current_data){
                                            current_val =mandate_current_data[key];                     
                                      }
                                  ...
                                  '<div class="row g-md-3">'+
                                    ...
                                    '<div class="col">'+
                                      '<div class="timeline-item-content ps-6 ps-md-3">'+
                                        '<h5 class="fs--1 lh-sm">'+action_shown +' Customer :<a href="<?php echo base_url('Users/transactionSchedule/');?>'+mandate_customer_id+'" class="sortable-item"> ' + customer_name + '</h5>'+
    ...
    if (logArray.length > 0) {
        ...
        for (var j = 0; j < logArray.length; j++) {
            ...
            html += '<p class="fs--1 text-800 mb-2">Details Change From: ' + JSON.stringify(log.mandate_previous_data) + '</p>';
            html += '<p class="fs--1 text-800 mb-2">To: ' + JSON.stringify(log.mandate_current_data) + '</p>';
    ...
    fetchAndDisplayMandateLogs(page);
});
